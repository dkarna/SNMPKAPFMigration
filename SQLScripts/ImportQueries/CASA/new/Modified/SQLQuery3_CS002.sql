...
,convert(varchar,GETDATE()+1, 105) next_int_run_date_dr
,' ' ledg_num
...
,CASE WHEN DATEDIFF(DAY,PDL.DueDate,getDate()) <= 30 THEN 'PASS'
WHEN (DATEDIFF(DAY,PDL.DueDate,getDate()) > 30) and (DATEDIFF(DAY,PDL.DueDate,getDate()) <= 90) THEN 'WATCH'
WHEN (DATEDIFF(DAY,PDL.DueDate,getDate()) > 90) and (DATEDIFF(DAY,PDL.DueDate,getDate()) <= 180) THEN 'SUBS'
WHEN (DATEDIFF(DAY,PDL.DueDate,getDate()) > 180) and (DATEDIFF(DAY,PDL.DueDate,getDate()) <= 365) THEN 'DBOUT'
WHEN DATEDIFF(DAY,PDL.DueDate,getDate()) > 365 THEN 'LOSS' ELSE 'PASS'
END AS health_code
...
,RIGHT(SPACE(3)+CAST('0' AS VARCHAR(3)),3) AS int_rate_prd_in_months
,RIGHT(SPACE(3)+CAST('0' AS VARCHAR(3)),3) AS int_rate_prd_in_days
,' ' penal_int_tbl_code
...
--) as t where t.int_tbl_code is null

--select top 10 * from #FinSchTbl

--select top 10 * from FinacleSchemeTable fst
--left join #FinSchTbl fstbl on fstbl.SCHM_CODE=fst.FinacleSchCode

--select * from FinacleLoanTable where FinacleSchCode in(select SCHM_CODE from 
--	FinacleCASAIntTableCode WHERE SCHM_CODE in
--	(select FinacleSchCode from FinacleLoanTable where FinacleSchemeType like 'ODA'))

--select * from Master where MainCode='033000000343C'